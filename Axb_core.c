...
long int a_sz, u_sz, s_sz, vt_sz, tot_sz;
LM_REAL thresh, one_over_denom;
register LM_REAL sum;
long int info, rank, worksz, *iwork, iworksz;
long int _m = m;
  ...
  worksz=5*_m; // min worksize for GESVD
  //worksz=m*(7*m+4); // min worksize for GESDD
#endif
  iworksz=8*_m;
  a_sz=_m*_m;
  u_sz=_m*_m; s_sz=_m; vt_sz=_m*_m;
  ...
  GESVD("A", "A", (int *)&_m, (int *)&_m, a, (int *)&_m, s, u, (int *)&_m, vt, (int *)&_m, work, (int *)&worksz, &info);
  ...
  for(rank=0, thresh=eps*s[0]; rank<_m && s[rank]>thresh; rank++){
    one_over_denom=LM_CNST(1.0)/s[rank];

    for(j=0; j<_m; j++)
      for(i=0; i<_m; i++)
        a[i*_m+j]+=vt[rank+i*_m]*u[j+rank*_m]*one_over_denom;
  }

	/* compute A^+ b in x */
	for(i=0; i<_m; i++){
	  for(j=0, sum=0.0; j<_m; j++)
      sum+=a[i*_m+j]*B[j];
    x[i]=sum;
  }
